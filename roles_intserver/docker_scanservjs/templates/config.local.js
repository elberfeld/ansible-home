...
module.exports = {
  ...
  afterConfig(config) {
    ...
    config.filename = () => {
      return `scan_${dayjs().format('YYYY-MM-DD_HH-mm')}`;
    };
  ...
  afterDevices(devices) {
    ...
    const device = devices.filter(d => d.id.startsWith('net:10.5.0.100:canon_lide70'))[0];
    if (device) {
      device.features['--resolution'].default = 300;
      device.features['-x'].default = 216;
      device.features['-y'].default = 297;
    }
  ...
  async afterScan(fileInfo) {
    ...
    // Sofort hochladen und löschen 
    // return await Process.spawn(`bash /etc/scanservjs/upload.sh '${fileInfo.fullname}' '${fileInfo.name}' `);
